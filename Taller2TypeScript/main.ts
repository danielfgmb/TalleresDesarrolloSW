function detalles_de(id:number) { 
...
class Serie {
    ...
    constructor(id: number,nombre: string, estudio: string,temporadas:number,sinopsis:string,url:string,img:string){
        this.id=id;
        this.nombre=nombre;
...
for (let i=0;i<series.length;i++){
    tabla?.insertAdjacentHTML('beforeend', '<tr onclick="detalles_de('+i+')" class="table-secondary"><th scope="row">'+series[i].id+'</th> <td>'+series[i].nombre+'</td> <td>'+series[i].estudio+'</td> <td>'+series[i].temporadas+'</td></tr>');
    sum+=series[i].temporadas;
} 

aver?.insertAdjacentHTML('beforeend', 'Seasons Average: '+Math.round(sum/series.length))
