"use strict";
function detalles_de(id) {
    card.innerHTML = '<img id="img-card" class="card-img-top" src="' + series[id].img + '"><div class="card-body" id="card-body"><h5 id="img-body" class="card-title">' + series[id].nombre + '</h5><p id="img-text" class="card-text">' + series[id].sinopsis + '</p><a  id="img-url" href="' + series[id].url + '" >' + series[id].url + '</a></div>';
}
class Serie {
    constructor(id, nombre, estudio, temporadas, sinopsis, url, img) {
        this.id = id;
        this.nombre = nombre;
...
for (let i = 0; i < series.length; i++) {
    tabla === null || tabla === void 0 ? void 0 : tabla.insertAdjacentHTML('beforeend', '<tr onclick="detalles_de(' + i + ')" class="table-secondary"><th scope="row">' + series[i].id + '</th> <td>' + series[i].nombre + '</td> <td>' + series[i].estudio + '</td> <td>' + series[i].temporadas + '</td></tr>');
    sum += series[i].temporadas;
}
aver === null || aver === void 0 ? void 0 : aver.insertAdjacentHTML('beforeend', 'Seasons Average: ' + Math.round(sum / series.length));
